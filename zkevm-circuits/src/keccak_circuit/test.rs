...
fn verify<F: Field>(k: u32, inputs: Vec<Vec<u8>>, success: bool) {
    ...
    let prover = MockProver::<F>::run::<_, true>(k, &circuit, vec![]).unwrap();
...
fn variadic_size_check() {
    ...
    let prover1 = MockProver::<Fr>::run::<_, true>(k, &circuit, vec![]).unwrap();
    ...
    let circuit = KeccakCircuit::new(num_rows, inputs);
    let prover2 = MockProver::<Fr>::run::<_, true>(k, &circuit, vec![]).unwrap();
